...
struct VS2PS
{
	...
	float2 P_LightScale_Fog : TEXCOORD2; // .x = LightScale; .y = Fog;
	float4 Color : COLOR0;
};
...
VS2PS Undergrowth_VS(APP2VS Input, uniform int LightCount, uniform bool ShadowMapEnable)
{
	...
	Output.P_LightScale_Fog.x = Input.Packed.w * 0.5;
	Output.P_LightScale_Fog.y = GetFogValue(Pos.xyz, _CameraPos.xyz);
...
) : COLOR
{
	...
	TerrainColor.rgb = lerp(TerrainColor.rgb, 1.0, Input.P_LightScale_Fog.x);
	...
	TerrainColor.rgb = ApplyFog(TerrainColor.rgb, Input.P_LightScale_Fog.y);
...
struct VS2PS_Simple
{
	...
	float4 P_TerrainColor_Fog : COLOR1; // .rgb = TerrainColor; .a = Fog;
};
...
VS2PS_Simple Undergrowth_Simple_VS(APP2VS_Simple Input, uniform int LightCount, uniform bool ShadowMapEnable)
{
	...
	if (ShadowMapEnable)
	{
		...
		Output.P_TerrainColor_Fog.rgb = lerp(Input.TerrainColorMap, 1.0, Input.Packed.w);
	}
	else
	{
		Light += Input.TerrainLightMap.y * _SunColor * 2.0;
		Output.P_TerrainColor_Fog.rgb = lerp(Input.TerrainColorMap, 1.0, Input.Packed.w);
		Output.P_TerrainColor_Fog.rgb *= Light;
	}

	Output.P_Light_Scale = saturate(Output.P_Light_Scale);
	Output.P_TerrainColor_Fog.rgb = saturate(Output.P_TerrainColor_Fog.rgb);
	Output.P_TerrainColor_Fog.a = GetFogValue(Pos.xyz, _CameraPos.xyz);
...
) : COLOR
{
	...
	if (ShadowMapEnable)
	{
		...
		LightColor = Base.rgb * Input.P_TerrainColor_Fog.rgb * Light * 2.0;
	}
	else
	{
		LightColor = Base.rgb * Input.P_TerrainColor_Fog.rgb * 2.0;
	}
	...
	OutColor.rgb = ApplyFog(OutColor.rgb, Input.P_TerrainColor_Fog.a);

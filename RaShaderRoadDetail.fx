...
VS2PS RoadDetail_VS(APP2VS Input)
{
...
float4 RoadDetail_PS(VS2PS Input) : COLOR
{
	float4 Color = tex2D(DiffuseMapSampler, Input.P_Tex0_Tex1.xy);
	Color = Color * tex2D(DetailMapSampler, Input.P_Tex0_Tex1.zw);
	float4 Light = 0.0;
...
technique defaultTechnique
{
	pass P0
	{
		VertexShader = compile vs_3_0 RoadDetail_VS();
		PixelShader = compile ps_3_0 RoadDetail_PS();
